...
def build_prompt(typhoon_data_text, location, language, style):
    ...
    task = (
        ...
        "颱風概況：[請參考此句型填空：颱風強度颱風中文名字，X日X時中心位於(方位)約(XXX)公里，以時速(XX)轉(XX)公里，向(XX)轉(XX)進行。其七級平均暴風半徑(XXX)公里；十級平均暴風半徑(XXX)公里。] (注意：請優先使用資料中提供距桃園市政府的數據；若資料中有提及『最大為XXX公里』再加括號補充，無則省略)\n"
