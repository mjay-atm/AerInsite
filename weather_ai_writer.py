...
def parse_args():
    ...
    parser.add_argument("--model", default="gemini-3.5-flash", help="模型名稱")
...
def build_prompt(weather_data_text, location, language, style):
    ...
    task = (
        ...
        f"{weather_data_text}"
        "注意上述五行需先行輸出，前後不要有任何文字或空行。完成後，先打印----分隔線----，接著撰寫你如何判斷降雨與風勢關注區域的邏輯，並說明你是如何從天氣資料中擷取出相關資訊的。"
    )
